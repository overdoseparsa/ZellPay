...
from abc import ABC , abstractmethod
class HandlerHash:
    def __init__(self, values: str, *args, **kwargs):
        self._values = values

        if args:
            self._values += ''.join(args)

        if kwargs:
            self._values += ''.join(f"{k}{v}" for k, v in kwargs.items())

        self._hash = SHA256.new(self._values.encode()).hexdigest()
    ...
    @classmethod
    def is_hash(cls, data: str, sha_hash: str) -> bool:
        return cls(data).data == sha_hash



class AbstractHashManager(
    ABC
):
    @classmethod
    def create_hash(
...
class Sha256Manger(
    ...
    @classmethod 
    def create_hash(
        cls , data:str ,*args,**kwargs 
    )->str:
        hander_hash = HandlerHash(
            data
        )
        return hander_hash.data
    
    @classmethod
    def check_hash(
        cls , data , hash
    )->bool:
        return HandlerHash.is_hash(
            data = data , 
            sha_hash=hash
        )